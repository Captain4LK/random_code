...
typedef struct
{
   ...
   uint8_t shades[64][256]; //Trying to improve cache hits by putting all colors of the same shade next to eachother
}HLH_palette;
...
void HLH_pal_calculate_colormap(HLH_palette *pal)
{
   for(int x = 0;x<256;x++)
   {
      for(int y = 0;y<64;y++)
      {
         ...
         {
            int r,g,b;
            r = MIN(255,(pal->colors[x].r*(63-y)+16)>>5);
            g = MIN(255,(pal->colors[x].g*(63-y)+16)>>5);
            b = MIN(255,(pal->colors[x].b*(63-y)+16)>>5);
...
{
   HLH_palette *pal = HLH_pal_load_from_file("test.cop");
   if(pal==NULL)
      puts(HLH_pal_get_error());
   HLH_pal_save("out.cop",pal);
   HLH_pal_free(pal);
   pal = HLH_pal_load_from_file("out.cop");
   if(pal==NULL)
      puts(HLH_pal_get_error());
   HLH_pal_free(pal);
